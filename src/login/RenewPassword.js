...
import { renewPassword } from '../actions/login/loginAction';
  ...
  contactSubmit = () => {
    if (this.checkPassword()) {
      if (this.state.fields.password === this.state.fields.confirmPassword) {
        ...
        renewPassword(this.state.fields.password, this.state.fields.current).then(res => {
          if (res && res.request && res && res.request.status === 401) {
            this.props.history.push('/login');
          } else if (res && res === 'Password is Updated') {
            this.setState({
              fields: {
                current: '',
                password: '',
                ...
                <form name="contactform" className="contactform">
                  <div className=" col-xs-12 ">
                    <div className="col-xs-12 form-group">
                      ...
                      <span style={{ color: 'red' }}>{this.state.errors['current']}</span>
                    </div>
