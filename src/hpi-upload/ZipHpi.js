...
import { handleUploadFileZip, handleRecentDataZip, handleUploadErrorZip } from '../actions/zip/ZipAction';
...
class ZipHpi extends React.Component {
  constructor(props) {
    super(props);
  ...
  componentDidUpdate = () => {
    if (this.props.hpiZipData && this.props.hpiZipData.length > 0 && !isTrue) {
      this.setState({
        isLoader: true
      });
      isTrue = true;
  ...
  recursive = () => {
    setTimeout(() => {
      let hasMore = this.state.items.length + 1 < this.props.hpiZipData.length;

      this.setState((prev, props) => ({
        items: props.hpiZipData.slice(0, prev.items.length + 1)
      }));
  ...
  handleRecentComponentClose = () => {
    this.props.handleRecentDataZip({}, false);
  };
  ...
  onChange(e) {
    ...
    if (!regex.test(e.target.value.toLowerCase()) && !regex2.test(e.target.value.toLowerCase())) {
      this.props.handleUploadErrorZip(true);
    } else {
      this.props.handleUploadFileZip(e.target.files[0], false);
      this.props.handleUploadErrorZip(false);
    }
  }
      ...
      fetch(url, {
        ...
        .then(res => {
          if (res && res.errorCode === 'upload Failed') {
            ...
            this.props.handleUploadFileZip('', false);
    ...
    fetch(myapi.RecentUpdateState, {
      ...
      .then(findresponse => {
        if (findresponse.isSuccess === 0) {
          ...
          this.props.handleRecentDataZip(findresponse, true);
        }
      })
      .catch(err => {
        error(err.message);
  ...
  replaceData = () => {
    if (this.state.matchData.length > 0) {
      ...
      const url = myapi.Replace_Upload_Data + 'zip'; //api to hit
          ...
          <div className="row">
            <div className="col-xs-12 pad-left-right-large">
              <div className="col-xs-12 upper-filter-section pad-top-30 nopad">
                <NavigationBar isZip={true} />
              </div>
              <div className="col-xs-12 pad-half white-bg curve-own pad-up-down">
                <div>
                  <ExportCsv type="zip" recursive={this.recursive} />
                </div>
                <div className="col-xs-12 text-center  white-bg">
                  ...
                  <form
                    ...
                    onSubmit={!this.props.noFileZip ? this.onFormSubmit : null}
                  ...
                  >
                    ...
                    {this.props.noFileZip ? (
                      <h4 className="col-xs-12 text-center text-danger">
                        Please select correct file format .csv or .txt.
                      </h4>
                    ) : null}
                    <div className="col-xs-12 text-center">
                      <button disabled={this.props.noFileZip} type="submit" className="btn orange-bg btn-own">
                        Upload
                      </button>
                    </div>
                    .
...
const mapStateToProps = state => {
  return {
    ...
    isRecentView: state.zipReducer.isRecentView,
    noFileZip: state.zipReducer.noFileZip,
    file: state.zipReducer.file,
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    handleRecentDataZip: (payload, value) => dispatch(handleRecentDataZip(payload, value)),
    handleUploadErrorZip: value => dispatch(handleUploadErrorZip(value)),
    error: value => dispatch(error(value)),
    success: value => dispatch(success(value)),
    handleUploadFileZip: (file, value) => dispatch(handleUploadFileZip(file, value))
  };
};
